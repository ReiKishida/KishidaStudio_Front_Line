...
#define	LOAD_FILENAME_TUTORIAL		("data/TEXT/NODE_DATA/NodeDataTutorial.txt")	// 読み込むファイルのパス
...
HRESULT CAIMecha::Init(void)
{
	...
	{// ノードの数だけ回る
	 // 差分を求める
		...
		{// 差分の最小値を求める
			fMinLength = fLength;
			m_nNearNode = nCntNode;
		}
	}
	m_nEndNode = m_nNearNode;
	m_nStartNode = m_nNearNode;
...
void CAIMecha::Update(void)
{
	...
	{// 生存時
		...
		{// マルチモード
			...
			if (CManager::GetClient()->GetPlayerIdx() == 0 && m_pPlayer->GetPlayerConnect() == false && m_pPlayer->GetDeath() == false)
			{
				// AI関係の更新処理
				CAIMecha::AIUpdate();
			}
			...
			{// AIの行動が決定している状態で左クリックされた場合
				...
				{// 自分を指定している場合
					...
					{// ポイントへの経路探索
						...
						// 経由地点の初期化
						for (int nCnt = 0; nCnt < NODEPOINT_MAX; nCnt++)
						{
							m_waypoint[nCnt] = m_pNodeData->GetLoadData().pos[m_nNearNode];
						}
		...
		{// ストラテジーパートでボタンマネージャーがNULLじゃない場合
			...
			{// AIの行動が決定している場合
				...
				{// 自分のメカタイプが選択されている場合
					...
					CManager::GetTutorial()->GetButtonManager()->GetSelectFinish() = false;	// 未決定状態に戻す
				}
			}
		}
	...
	{// 終了ノードの設定
		...
		{// ノードの数だけ回る
		 // 差分を求める
			...
			{
				fLength =
					(m_pNodeData->GetLoadData().pos[nCntNode].x - CManager::GetTutorial()->GetMouse()->Getsetpos().x) *
					(m_pNodeData->GetLoadData().pos[nCntNode].x - CManager::GetTutorial()->GetMouse()->Getsetpos().x) +
					(m_pNodeData->GetLoadData().pos[nCntNode].z - CManager::GetTutorial()->GetMouse()->Getsetpos().z) *
					(m_pNodeData->GetLoadData().pos[nCntNode].z - CManager::GetTutorial()->GetMouse()->Getsetpos().z);
			}

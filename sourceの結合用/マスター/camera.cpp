...
// マクロ定義
//*****************************************************************************
#define CAMERA_LENGTH		(-80.0f)		// カメラの距離
	...
	{//モードがゲームの場合
		...
		if (CManager::GetGame()->GetGameState() == CGame::STATE_NORMAL || CManager::GetGame()->GetGameState() == CGame::STATE_STARTING_FADE_IN)
		{
			...
			{// ストラテジーパート
				...
				{// 左CTRLと左クリックでスクロール
					m_posVDest.x -= (moveX / m_fZoom) * 4.0f;
					m_posVDest.z += (moveY / m_fZoom) * 4.0f;

					m_posRDest.x -= (moveX / m_fZoom) * 4.0f;
					m_posRDest.z += (moveY / m_fZoom) * 4.0f;
				}
...
void CCamera::Set(int nIdxCamera)
{
	...
	{// プロジェクションマトリックスを作成
		...
		{// ストラテジーパート
			...
			{
				D3DXMatrixOrthoLH(&m_mtxProjection,
					(float)m_viewport.Width /  PARALLEL_PROJECTION_SIZE,
					(float)m_viewport.Height / PARALLEL_PROJECTION_SIZE,
					1.0f,
					1.0f);
			}
			else

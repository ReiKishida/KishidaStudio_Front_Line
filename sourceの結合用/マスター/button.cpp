...
#define BUTTON_LOGIC_WIDTH			(140.0f)									// ロジックツリーボタンの幅
#define BUTTON_LOGIC_HEIGHT			(140.0f)									// ロジックツリーボタンの高さ
...
bool CButton2D::InRangeMenu(D3DXVECTOR3 _pos)
{
	...
	if (pos.x - (m_size.x * 0.35f) <= _pos.x && pos.x + (m_size.x * 0.35f) >= _pos.x)
	{// X軸が範囲内
		if (pos.y - (m_size.y * 0.35f) <= _pos.y && pos.y + (m_size.y * 0.35f) >= _pos.y)
		{// Y軸が範囲内
			m_bInRange = true;
			CScene2D::SetColor(D3DXCOLOR(1.0f, 1.0f, 1.0f, 1.0f));
			return m_bInRange;
...
void CButtonLine::Update(void)
{
	...
	{
		cursor = CManager::GetGame()->GetMouseCursor()->GetPos();
	}
	else if (CManager::GetTutorial() != NULL)
	{
		cursor = CManager::GetTutorial()->GetMouseCursor()->GetPos();
	}
	// カーソル位置への角度を計算
...
void CButtonManagerStrategy::Uninit(void)
{
	...
	if (m_pMarker != NULL)
	{
		...
		if (0 < m_aSelectLogic[2]) { nNumMarker = 4; }
		else { nNumMarker = 1; }
	...
	{// マーカーの設置
		...
		{// 左クリックされた
			...
			{// マーカーを置く
				...
				{
					m_pMarker[m_nSetNumMarker]->SetPos(CManager::GetTutorial()->GetMouse()->GetPos());
				}
				m_nSetNumMarker++;
			}
			else
			{
				...
				{
					m_pMarker[m_nSetNumMarker - 1]->SetPos(CManager::GetTutorial()->GetMouse()->GetPos());
				}
			}
		}
	}
...
void CButtonManagerStrategy::AITypeSelect(void)
{
	...
	{// ドローンとウォーカーを両方チェック
			...
			{// 範囲内かチェック
				...
				{// クリックされた
					...
					{// 起動時
						...
						{// ボタンの生成
							float posY = (475.0f + (nCntButton * BUTTON_LOGIC_HEIGHT * (4 - m_aNumLogic[0] * 1.25f))) - (BUTTON_LOGIC_HEIGHT * (4 - m_aNumLogic[0] * 1.25f) * (m_aNumLogic[0] / 2.0f)) + BUTTON_LOGIC_HEIGHT * (4 - m_aNumLogic[0] * 1.25f) / 2.0f;
							m_pFirst[nCntButton] = CButton2D::Create(D3DXVECTOR3(600.0f, posY, 0.0f), BUTTON_LOGIC_WIDTH, BUTTON_LOGIC_HEIGHT);
		...
		{
			if (m_pAIType[nCntAIType]->InRange(CManager::GetTutorial()->GetMouseCursor()->GetPos()))
			{// 範囲内かチェック
				if (m_pAIType[nCntAIType]->ClickRelease())
				{// クリックされた
					ButtonUninit(4);	// ボタンの破棄
...
void CButtonManagerStrategy::FirstHierarchy(void)
{
	for (int nCntFirst = 0; nCntFirst < m_aNumLogic[0]; nCntFirst++)
	{
				...
				{// クリックされた
					...
					{// 起動時
						...
						{// 0番目のボタン
							...
							{// ボタンの生成
								float posY = (475.0f + (nCntButton * BUTTON_LOGIC_HEIGHT * (4 - m_aNumLogic[1] * 1.25f))) - (BUTTON_LOGIC_HEIGHT * (4 - m_aNumLogic[1] * 1.25f) * (m_aNumLogic[1] / 2.0f)) + BUTTON_LOGIC_HEIGHT * (4 - m_aNumLogic[1] * 1.25f) / 2.0f;
						...
						{// 選択した項目の生成
							...
							m_apSelectIcon[0]->SetSize(BUTTON_LOGIC_WIDTH * 0.7f, BUTTON_LOGIC_HEIGHT * 0.7f);
		...
		{
			if (m_pFirst[nCntFirst]->InRange(CManager::GetTutorial()->GetMouseCursor()->GetPos()))
			{// 範囲内かチェック
				if (m_pFirst[nCntFirst]->ClickRelease())
				{// クリックされた
					ButtonUninit(3);	// ボタンの破棄
...
void CButtonManagerStrategy::SecondHierarchy(void)
{
	for (int nCntSecond = 0; nCntSecond < m_aNumLogic[1]; nCntSecond++)
	{
					...
					{// 起動時
						...
						{// 移動ロジックツリー
							...
							{// 0番目のボタン
								...
								{// ボタンの生成
									float posY = (475.0f + (nCntButton * BUTTON_LOGIC_HEIGHT * (4 - m_aNumLogic[2] * 0.98f))) - (BUTTON_LOGIC_HEIGHT * (4 - m_aNumLogic[2] * 0.98f) * (m_aNumLogic[2] / 2.0f)) + BUTTON_LOGIC_HEIGHT * (4 - m_aNumLogic[2] * 0.98f) / 2.0f;
							...
							{// 1番目のボタン
								...
								for (int nCntButton = 0; nCntButton < m_aNumLogic[2]; nCntButton++)
								{// ボタンの生成
									float posY = (475.0f + (nCntButton * BUTTON_LOGIC_HEIGHT * (4 - m_aNumLogic[2] * 1.25f))) - (BUTTON_LOGIC_HEIGHT * (4 - m_aNumLogic[2] * 1.25f) * (m_aNumLogic[2] / 2.0f)) + BUTTON_LOGIC_HEIGHT * (4 - m_aNumLogic[2] * 1.25f) / 2.0f;
									m_pThird[nCntButton] = CButton2D::Create(D3DXVECTOR3(992.0f, posY, 0.0f), BUTTON_LOGIC_WIDTH, BUTTON_LOGIC_HEIGHT);
									m_pThird[nCntButton]->BindTexture(CTexture::GetTexture((CTexture::TEXTURE)(CTexture::TEXTURE_STRATEGY_ICON)));
						...
						{// 待機ロジックツリー
							...
							{// ボタンの生成
								float posY = (475.0f + (nCntButton * BUTTON_LOGIC_HEIGHT * (4 - m_aNumLogic[2] * 1.25f))) - (BUTTON_LOGIC_HEIGHT * (4 - m_aNumLogic[2] * 1.25f) * (m_aNumLogic[2] / 2.0f)) + BUTTON_LOGIC_HEIGHT * (4 - m_aNumLogic[2] * 1.25f) / 2.0f;
								m_pThird[nCntButton] = CButton2D::Create(D3DXVECTOR3(1180.0f, posY, 0.0f), BUTTON_LOGIC_WIDTH, BUTTON_LOGIC_HEIGHT);
								m_pThird[nCntButton]->BindTexture(CTexture::GetTexture((CTexture::TEXTURE)(CTexture::TEXTURE_STRATEGY_ICON)));
						...
						{// 選択した項目の生成
							...
							m_apSelectIcon[1]->SetSize(BUTTON_LOGIC_WIDTH * 0.7f, BUTTON_LOGIC_HEIGHT * 0.7f);
		...
		{
			if (m_pSecond[nCntSecond]->InRange(CManager::GetTutorial()->GetMouseCursor()->GetPos()))
			{// 範囲内かチェック
				if (m_pSecond[nCntSecond]->ClickRelease())
				{// クリックされた
					ButtonUninit(2);	// ボタンの破棄
...
void CButtonManagerStrategy::ThirdHierarchy(void)
{
	...
	for (int nCntThird = 0; nCntThird < m_aNumLogic[2]; nCntThird++)
	{
				...
				{// クリックされた
					...
					{// 起動時
						...
						{// 選択した項目の生成
							...
							m_apSelectIcon[2]->SetSize(BUTTON_LOGIC_WIDTH * 0.7f, BUTTON_LOGIC_HEIGHT * 0.7f);
						...
						{// 移動ロジックツリー
							...
							{// ボタンの生成
								float posY = (475.0f + (nCntButton * BUTTON_LOGIC_HEIGHT * (4 - m_aNumLogic[3] * 1.25f))) - (BUTTON_LOGIC_HEIGHT * (4 - m_aNumLogic[3] * 1.25f) * (m_aNumLogic[3] / 2.0f)) + BUTTON_LOGIC_HEIGHT * (4 - m_aNumLogic[3] * 1.25f) / 2.0f;
								m_pFourth[nCntButton] = CButton2D::Create(D3DXVECTOR3(1180.0f, posY, 0.0f), BUTTON_LOGIC_WIDTH, BUTTON_LOGIC_HEIGHT);
		...
		{
			if (m_pThird[nCntThird]->InRange(CManager::GetTutorial()->GetMouseCursor()->GetPos()))
			{// 範囲内かチェック
				if (m_pThird[nCntThird]->ClickRelease())
				{// クリックされた
					ButtonUninit(1);	// ボタンの破棄
...
void CButtonManagerStrategy::FourthHierarchy(void)
{
	...
	for (int nCntFourth = 0; nCntFourth < m_aNumLogic[3]; nCntFourth++)
	{
			...
			{// 範囲内かチェック
				...
				{// クリックされた
					...
					{// 選択した項目の生成
						...
						m_apSelectIcon[3]->SetSize(BUTTON_LOGIC_WIDTH * 0.7f, BUTTON_LOGIC_HEIGHT * 0.7f);
		...
		{
			if (m_pFourth[nCntFourth]->InRange(CManager::GetTutorial()->GetMouseCursor()->GetPos()))
			{// 範囲内かチェック
				if (m_pFourth[nCntFourth]->ClickRelease())
				{// クリックされた
					m_aSelectLogic[3] = nCntFourth;

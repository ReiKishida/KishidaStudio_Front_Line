...
public:
	typedef enum
	{
		...
		STATE_STARTING,	// 開始前
		STATE_STARTING_FADE_OUT,
		STATE_STARTING_FADE_IN,
		STATE_NORMAL,	// 通常状態
		STATE_END,		// 終了
		STATE_END_FADE_OUT,
		STATE_FADE,		// フェードする状態
	...
	CModel *GetFloor(void) { return m_pFloor; };
...
private:
	...
	void SetPinData(int nPlayerIdx, D3DXVECTOR3 pinPos, bool use);
	void SetAIPinData(int nPlayerIdx, int AIIdx, D3DXVECTOR3 AIPinPos, bool use);
	...
	void StartingFadeOutUpdate(void);
	void StartingFadeInUpdate(void);
	void NomalUpdate(void);
	void EndUpdate(void);
	void EndFadeOutUpdate(void);
	...
	CUI_TEXTURE *m_pFadeBG;

	CUI_TEXTURE *m_pEndBG;
	CUI_TEXTURE *m_pEndResultLogo;
	CUI_TEXTURE *m_pEndMyTeamLogo;

	CUI_NUMBER *m_pEndScore[NUM_TEAM];
	CUI_TEXTURE *m_pEndTeamEmbrem[NUM_TEAM];

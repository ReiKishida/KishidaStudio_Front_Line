...
int CUI_NUMBER::m_nDroneLife = 0;
int CUI_NUMBER::m_nWorkerLife = 0;
...
HRESULT CUI_NUMBER::Init(void)
{
	...
	case UI_NUMTYPE_DRONE_HP:
		SetNumDisPlay(m_nDroneLife, m_UINumType);		// ライフを設定
		break;

	case UI_NUMTYPE_WORKER_HP:
		SetNumDisPlay(m_nWorkerLife, m_UINumType);		// ライフを設定
		break;
		...
		if (CMenu::GetMode() == CMenu::MODE_MULTI)
		{
			if (CManager::GetClient() != NULL)
			{
				if (CManager::GetClient()->GetConnect() == true)
				{
					...
					if (m_nDroneLife >= 0)
					{	// 0以上の時、数字更新
						SetNum(m_nDroneLife, CGame::GetPlayer(CManager::GetClient()->GetPlayerIdx())->GetLifeMax(), m_col);
					}
				}
			}
			else
			{
				...
				if (m_nDroneLife >= 0)
				{	// 0以上の時、数字更新
					SetNum(m_nDroneLife, CGame::GetPlayer(0)->GetLifeMax(), m_col);
				}
			}
		}
		else if (CMenu::GetMode() == CMenu::MODE_SINGLE)
		{
			SetNumColor(m_nDroneLife, CGame::GetPlayer(0)->GetMyAI(0)->GetLife());

			if (m_nDroneLife >= 0)
			{	// 0以上の時、数字更新
				SetNum(m_nDroneLife, CGame::GetPlayer(0)->GetMyAI(0)->GetLife(), m_col);
			}
		}
		break;
		...
		if (CMenu::GetMode() == CMenu::MODE_MULTI)
		{
			if (CManager::GetClient() != NULL)
			{
				if (CManager::GetClient()->GetConnect() == true)
				{
					...
					if (m_nWorkerLife >= 0)
					{	// 0以上の時、数字更新
						SetNum(m_nWorkerLife, CGame::GetPlayer(CManager::GetClient()->GetPlayerIdx())->GetLifeMax(), m_col);
					}
				}
			}
			else
			{
				...
				if (m_nWorkerLife >= 0)
				{	// 0以上の時、数字更新
					SetNum(m_nWorkerLife, CGame::GetPlayer(0)->GetLifeMax(), m_col);
				}
			}
		}
		else if (CMenu::GetMode() == CMenu::MODE_SINGLE)
		{
			SetNumColor(m_nWorkerLife, CGame::GetPlayer(0)->GetMyAI(1)->GetLife());

			if (m_nWorkerLife >= 0)
			{	// 0以上の時、数字更新
				SetNum(m_nWorkerLife, CGame::GetPlayer(0)->GetMyAI(1)->GetLife(), m_col);
			}
		}
		break;
	}

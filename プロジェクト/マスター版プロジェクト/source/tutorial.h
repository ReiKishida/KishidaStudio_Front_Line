...
// マクロ定義
//*****************************************************************************
#define TUTORIAL_PRIORITY		(6)				// 優先番号
...
public:
	...
	CTutorial(int nPriority = TUTORIAL_PRIORITY, CScene::OBJTYPE objType = CScene::OBJTYPE_GAME);
	~CTutorial();
	HRESULT Init(void);
	void Uninit(void);
	void Update(void);
	...
	CModel *GetClearMarker(void) { return m_pClearMarker; };
...
private:
	...
	CModel *m_pClearMarker;						// クリアマーカーのモデル
	...
	CLEARSTATE m_ReloadState;	//リロードのクリア状態
								...
								//ストラテジーパートのクリア状態
	CLEARSTATE m_Ai_Move_State;			//移動のクリア状態
	CLEARSTATE m_Ai_Attack_State;		//攻撃のクリア状態

...
void CShadow::Set(D3DXMATRIX mtxWorld, LPD3DXMESH mesh, int nNumMat, D3DXVECTOR3 _pos, float fHeight)
{
	...
	{
		if (CManager::GetGame()->GetPart() == CGame::PART_STRATEGY) { return; }
	}
	else if (CManager::GetTutorial() != NULL)
	{
		if (CManager::GetTutorial()->GetPart() == CGame::PART_STRATEGY) { return; }
	}
	m_ObjPos = _pos;
	...
	// 影の位置
	if (CGame::STATE_STARTING_FADE_OUT == CManager::GetGame()->GetGameState() || CGame::STATE_STARTING == CManager::GetGame()->GetGameState())
	{
		pos = D3DXVECTOR3(0.0f, 6.0f, 0.0f);
	}
	else
	{

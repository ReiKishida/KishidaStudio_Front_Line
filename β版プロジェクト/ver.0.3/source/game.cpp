...
void CGame::EndFadeOutUpdate(void)
{
	...
	{//NULLではない場合
		...
		{//透明度が0.5以上の場合
			...
			{//NULLの場合
				//ブルーチームのスコアを生成
				m_pEndScore[0] = CUI_NUMBER::Create(D3DXVECTOR3(325.0f, 500.0f, 0.0f), 150.0f, 150.0f, 75.0f, CUI_NUMBER::UI_NUMTYPE_NONE, 0, 1, 3);
			...
			{//NULLの場合
				//レッドチームのスコアを生成
				m_pEndScore[1] = CUI_NUMBER::Create(D3DXVECTOR3(830.0f, 500.0f, 0.0f), 150.0f, 150.0f, 75.0f, CUI_NUMBER::UI_NUMTYPE_NONE, 0, 1, 3);
...
void CGame::CreatePlayer(void)
{
	for (int nCntPlayer = 0; nCntPlayer < MAX_PLAYER_CONNECT; nCntPlayer++)
	{
		...
		{//シングルプレイの場合
			...
			{//それ以外の場合
				//m_aMechaType[nCntPlayer] = (CMechaSelect::MECHATYPE)(rand() % CMechaSelect::MECHATYPE_MAX);
				m_aMechaType[nCntPlayer] = CMechaSelect::MECHATYPE_SNIPE;
